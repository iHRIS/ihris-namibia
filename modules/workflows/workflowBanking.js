...
const workflowBanking = {
  process: ( req ) => {
    return new Promise( (resolve, reject) => {
      fhirQuestionnaire.processQuestionnaire( req.body ).then( (bundle) => {
        let practitioner = req.query.practitioner
        let bankingIndex = bundle.entry && bundle.entry.findIndex(entry => entry.resource.resourceType === 'Basic' && entry.resource.meta && entry.resource.meta.profile.includes('http://ihris.org/fhir/StructureDefinition/ihris-basic-banking'))
        let banking = bundle.entry[bankingIndex]
        banking.resource.extension.push({
...
module.exports = workflowBanking
